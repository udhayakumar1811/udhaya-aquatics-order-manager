...
export default function Dashboard({ setActiveTab }) {
  ...
  const pendingOrders = orders.filter(o => (o.status || o.orderStatus) === 'Pending');
  const shippedOrders = orders.filter(o => (o.status || o.orderStatus) === 'Shipped');
    ...
    <div className="p-4 max-w-7xl mx-auto space-y-4 text-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex justify-between items-center">
          ...
          <div className="w-10 h-10 bg-amber-50 text-amber-500 rounded-lg flex items-center justify-center text-base">⏳</div>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex justify-between items-center">
          ...
          <div className="w-10 h-10 bg-purple-50 text-purple-600 rounded-lg flex items-center justify-center text-base">🚚</div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex justify-between items-center">
          ...
          <div className="w-10 h-10 bg-indigo-50 text-indigo-600 rounded-lg flex items-center justify-center text-base">₹</div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse text-xs">
            ...
            <tbody className="divide-y divide-gray-100 text-gray-700">
              {loading ? (
                <tr><td colSpan="6" className="text-center py-4 text-gray-400">Loading orders...</td></tr>
              ) : orders.length === 0 ? (
                <tr><td colSpan="6" className="text-center py-4 text-gray-400">No orders found yet. Add your first order!</td></tr>
              ) : (
                    ...
                    <tr key={order.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="py-3 px-4">
                        <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-medium ${
                          currentStatus === 'Delivered' ? 'bg-emerald-50 text-emerald-600' :
                          currentStatus === 'Shipped' ? 'bg-purple-50 text-purple-600' :
